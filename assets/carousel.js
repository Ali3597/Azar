...
class Carousel {
  ...
  constructor(element, options = {}) {
    ...
    if (this.options.style === "promo") {
      this.root.addEventListener(
        "mouseover",
        this.makeArrowsVisible.bind(this)
      );
      this.root.addEventListener(
        "mouseout",
        this.makeArrowsInvisible.bind(this)
      );
    }
  ...
  createNavigation() {
    this.nextButton = this.createDivWithClass(
      `carousel_next_${this.options.style}`
    );
    if (this.options.style == "promo") {
      this.nextButton.innerHTML = `<i class=" fa-2x fas fa-chevron-right"></i>`;
    } else {
      this.nextButton.innerHTML = `<i class=" fa-lg fas fa-arrow-right"></i>`;
    }

    this.prevButton = this.createDivWithClass(
      `carousel_prev_${this.options.style}`
    );
    if (this.options.style == "promo") {
      this.prevButton.innerHTML = `<i class="fa-2x fas fa-chevron-left"></i>`;
    } else {
      this.prevButton.innerHTML = `<i class=" fa-lg fas fa-arrow-left"></i>`;
    }
  ...
  createPagination() {
    ...
    if (this.options.style == "produit") {
      ...
      let pagination = this.createDivWithClass(
        `carousel_pagination_${this.options.style}`
      );
      let buttons = [];
      ...
      for (
        ...
        let button = this.createDivWithClass(
          `carousel_pagination_button_${this.options.style}`
        );
        ...
        if (activeButton) {
          buttons.forEach((button) =>
            button.classList.remove(
              `carousel_pagination_button_active_${this.options.style}`
            )
          );
